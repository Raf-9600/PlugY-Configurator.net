...
#region "Methods"
        ...
        private bool ParseKeyValuePair(string Line, ref string Key, ref string Value)
        {
            ...
            Value = (j > 0) ? (Line.Substring(i + 1, j)) : ("");
            return true;
        }
		...
		internal void Refresh()
		{
			lock (m_Lock)
			{
				...
				try
				{
					...
					try
					{
                        sr = new StreamReader(m_FileName, Encoding.ASCII);
					}
        ...
        private void PerformFlush()
        {
            ...
            sw = new StreamWriter(TmpFileName, false, Encoding.ASCII);
		...
		internal string GetValue(string SectionName, string Key, string DefaultValue = null)
		{
			// *** Lazy loading ***
			if (m_Lazy)
			{
